...
st.markdown("""
    ...
    .time-box { background-color: #1e2129; padding: 15px; border-radius: 10px; border-left: 5px solid #00ff00; margin-bottom: 20px; }
    .payment-box { background-color: #1a1c23; padding: 15px; border-radius: 10px; border-left: 5px solid #ff4b4b; }
    .bank-details { background-color: #ffffff; color: #000000; padding: 20px; border-radius: 10px; border: 2px solid #ff4b4b; margin-bottom: 20px; }
    </style>
...
with tab1:
    ...
    with st.form("booking_form"):
        ...
        is_urgent = (start_dt - datetime.now()).total_seconds() / 3600 < URGENCY_THRESHOLD_HOURS
        base_price = URGENT_BASE if is_urgent else STANDARD_BASE
        total_cost = base_price + (max(0, duration - 2) * EXTRA_HOUR_FEE)
        deposit_amount = total_cost * DEPOSIT_PERCENTAGE

        st.markdown(f"<div class='time-box'><h4>Window: {start_dt.strftime('%I:%M %p')} — {end_dt.strftime('%I:%M %p')}</h4></div>", unsafe_allow_all_html=True)
        st.markdown(f"<div class='payment-box'><p>Total: ${total_cost}</p><h3>Deposit Required: ${deposit_amount}</h3></div>", unsafe_allow_all_html=True)
...
with tab2:
    ...
    st.markdown(f"<div class='bank-details'><h4 style='color:black; margin-top:0;'>Bank Transfer Details:</h4><p><b>Acc Name:</b> {ACCOUNT_NAME}<br><b>BSB:</b> {BSB_NUMBER}<br><b>Acc #:</b> {ACCOUNT_NUMBER}</p></div>", unsafe_allow_all_html=True)
